...
#item(
  ...
  硕士，计算机技术｜#link("https://vrlab.buaa.edu.cn/")[VR Lab]，导师：#mask(mark: true)[#resume_advisor]｜研究方向为*计算机视觉-视频动作识别*。 #linebreak()
]
#item(
  tl: [北京航空航天大学],
  tr: [2019.09-2023.06],
...
#item(
  ...
  - 负责AI工作流Windows端应用的后台服务开发上线：实现主对话框文件上传后自动返回推荐提示词，完成Deepseek云服务迁移，支持KOL邀请码生成与验证，集成算法解析图文/视频URL以获取文本与图像内容，接入调用hunyuan-turbo与hunyuan-vision模型的能力，基于MCP协议打通LLM工具调用链路用于推荐论文；
  - 完善应用后台的SRE可观测性监控：补充主调、被调与事件监控，强化服务运行状态的可观测性与问题定位效率。
]
...
#item(
  ...
  - 协助公司服务管理平台的前后端开发与维护：基于Vue前端和Gin后端开发需求（如开发镜像管理及详情页面）；修复性能问题（如改进可过滤表格页面query与filter变量ref同步逻辑）；优化交互体验（如应用登记后实时返回CI/CD流水线状态）。整体提升系统性能与用户满意度；
  - 搭建业务数据分析与可视化能力：利用DataEase BI与Doris数据库构建运营数据仪表盘，为业务提供实时决策支持。
]
...
#item(
  ...
  - 基于Django框架构建3D高斯泼溅网页应用，为非专业用户实现从视频到三维场景的自动转化；
  // - 通过Gradio和云端GPU部署Llama3-8B-Instruct Chatbot；
  - 使用FastAPI框架搭建Stable Diffusion LoRA文生图模型风格微调平台后端，提升模型微调迭代效率。
]
...
#item(
  ...
  - 用 Golang编程解析楼宇自控设备通信BACnet协议报文参数并模拟，使用cobra将其包装成命令行工具；
]
...
#grid(
  ...
  row-gutter: 11pt,
  gutter: 8pt,
  [*编程语言*], [熟悉Golang, Python, JavaScript等；学习并使用过C, C++, Java等（可快速上手）；],
  [*应用开发*],
  [熟悉Gin, Django等后端框架和Vue.js前端框架，具备MySQL数据库开发与设计经验，能够结合业务场景应用Redis缓存与Kafka消息队列，了解RPC微服务架构开发流程，使用过Docker, K8s等微服务运维工具，并能熟练使用生成式AI工具提高工作效率；],
  [*主修课程*],
